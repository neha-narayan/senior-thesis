...
// 	drop if mi(govt_ind)
	...
	keep c1_tot* c2_tot* c3_tot* c4_tot* c5_tot* c6_tot* c7_tot* c8_tot* statename distname ac_year schtot
	collapse (sum) c1_tot* c2_tot* c3_tot* c4_tot* c5_tot* c6_tot* c7_tot* c8_tot* (firstnm) schtot, by(state distname ac_year) fast
...
// 	drop if mi(govt_ind)
    ...
    bysort state district ac_year: gen N = _N
	rename N schtot

	collapse (sum) c1_tot* c2_tot* c3_tot* c4_tot* c5_tot* c6_tot* c7_tot* c8_tot* (firstnm) schtot, by(state district ac_year) fast
...
program merge_district_panels
	...
	//andaman + nicobar
	replace distname = "ANDAMANS" if distname == "SOUTH ANDAMANS"
	//arunachal pradesh
	...
	replace distname = "BASTAR" if distname == "KONDAGAON" | distname == "NARAYANPUR"
	...
	replace distname = "KAKCHING" if distname == "THOUBAL"
	...
	replace distname = "NONEY" if distname == "TAMENGLONG"
	...
	qui ds statename distname ac_year, not
	collapse (sum) `r(varlist)', by(statename distname ac_year)
...
program population_scaling 
	...
	merge m:1 statename using ../../../shared_data/shares_from_2011, assert(1 2 3) keep(3)
	drop _merge
	qui ds c*
	...
	collapse (sum) enrollment enrollment_g enrollment_b schtot ///
	    (firstnm) primaryage_all primaryage_female primaryage_male, ///
	    by(statename ac_year)
	gen enrollment_rate = enrollment/primaryage_all
	gen enrollment_rate_g = enrollment_g/primaryage_female
	gen enrollment_rate_b = enrollment_b/primaryage_male
	
// 	gen sus_enroll_ind = 0
// 	replace sus_enroll_ind = 1 if enrollment_rate > 1 
//	
