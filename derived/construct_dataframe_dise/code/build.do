...
program append_files_pre_2017
	...
	forvalues year = 2005/2017 {
	    import delimited "../output/csv/basic_`year'", varnames(1)
	    cap rename schcd school_code 
	...
	*/
	...
	clear
	forvalues year = 2005/2017 {
	    import delimited "../output/csv/general_`year'", varnames(1) 
	    cap rename schcd school_code 
	    cap replace schtype = "" if schtype == "NA"
	    cap replace rururb = "" if rururb == "NA"
	    cap destring schtype rururb, replace
	...
	clear
	forvalues year = 2005/2017 {
	    import delimited "../output/csv/facility_`year'", varnames(1)
	    cap rename schcd school_code
	    cap rename (classrooms_in_good_condition classrooms_require_major_repair) (clgood clmajor)
	    cap rename (classrooms_require_minor_repair other_rooms_in_good_cond) (clminor othgood)
	    cap rename (other_rooms_need_major_rep other_rooms_need_minor_rep) (othmajor othminor)
	    cap rename (tch_with_professional days_involved_in_non_tch) (tchwithprof daysinvld)
        cap rename (teachers_involved_in_non_tch_ass) (tchinvld) 
	...
	forvalues year = 2009/2017 {
	    import delimited "../output/csv/teachers_`year'", varnames(1)
	    cap rename schcd school_code  
	...
	forvalues year = 2010/2017 {
	    import delimited "../output/csv/rte_`year'", varnames(1) 
	    cap rename schcd school_code
	    cap rename (approachable_by_all_weather_road smc_members_parents_male smc_members_parents_female) ///
	       (approachbyroad smsparents_m smsparents_f)
	    cap rename (smc_members_local_authority_male smc_members_local_authority_fema smc_meetings_held) ///
	        (smcnomlocal_m smcnomlocal_f smcmeetings)
	    cap rename (school_developmentplan_prepared smc_children_record_maintained) (smcsdp_yn smschildrec_yn)
	    cap rename (text_book_received_month text_book_received_year) (txtbkmnth txtbkyear) 
	    cap rename (people_cumil pcr_shared_with_parents children_from_weaker_section_app) ///
	        (pcr_maintained pcr_shared wsec25p_applied)
	    cap rename (children_from_weaker_section_enr chld_enrolled_for_sp_training_cu) ///
	        (wsec25p_enrolled spltrg_cy_enrolled_b) 
	    cap rename (spl_training_provided_current_ye spl_training_enrolled_previous_y) ///
	        (spltrg_cy_provided_b spltrg_py_enrolled_b)
	    cap rename (spl_training_provided_previous_y spl_training_conducted_by) ///
	        (spltrg_py_provided_b spltrg_by)
	    cap rename chilren_admitted_for_free admitted_free
		tempfile rte_`year'
		save "`rte_`year''"
		clear
	}
	...
	clear
	forvalues year = 2005/2017 {
	    import delimited "../output/csv/scenrollment_`year'", varnames(1) 
	    cap rename schcd school_code 
	    cap qui ds class*
	    foreach var in `r(varlist)' {
		    cap local class = substr("`var'", 6, 1)
		    cap local gender = substr("`var'", 15, 1)
		    cap dis "`class' `gender'"
		    replace `var' = "" if `var' == "NA"
		    cap rename `var' c`class'_c`gender' 
	    }
		convert_to_int
	...
	clear
	forvalues year = 2005/2017 {
	    import delimited "../output/csv/stenrollment_`year'", varnames(1)
	    cap rename schcd school_code 
	    cap qui ds class*
	    foreach var in `r(varlist)' {
		    cap local class = substr("`var'", 6, 1)
		    ...
		    cap dis "`class' `gender'"
		    replace `var' = "" if `var' == "NA"
		    cap rename `var' c`class'_t`gender'
	    }
		destring c*, replace
	...
	clear
	forvalues year = 2005/2017 {
	    import delimited "../output/csv/obcenrollment_`year'", varnames(1) 
	    cap rename schcd school_code
	    cap qui ds class*
	    foreach var in `r(varlist)' {
		    cap local class = substr("`var'", 6, 1)
		    cap local gender = substr("`var'", 16, 1)
		    cap dis "`class' `gender'"
		    replace `var' = "" if `var' == "NA"
		    cap rename `var' c`class'_o`gender' 
	    } 
		convert_to_int
	...
	forvalues year = 2005/2017 {
	    ...
	    cap qui ds disabled*
	    foreach var in `r(varlist)' {
		    cap local class = substr("`var'", 11, 1)
		    cap local gender = substr("`var'", 13, 1)
		    cap dis "`class' `gender'"
		    replace `var' = "" if `var' == "NA"
		    cap rename `var' c`class'_dis_`gender' 
	    }
		convert_to_int
...
program recode_appended
	...
	rename kitchen_devaices_grant kitdevgrant_yn
	save "../output/messy_dta/rte_append", replace
